...
const int maxPower = 94; //delay(2000) gives a complete 180
const int midPower = 67; //delay(1000) gives a nice 90
const int minPower = 47;
...
const int IRInputPin = 1; // Analog in
  ...
  if (messageSize > 0) {
    ...
    if(message.endsWith("Sizz")){
      ...
      //   }
        ...
        testColor();
      }else if(message.endsWith("testWallSizz")){
        testWall();
      }
    }
  }
...
void botDemo(int demoMode){
  if(demoMode == 4){
    ...
    while(!message.endsWith("JAQPETTI FOUND RED")){
  ...
  for(int i = 0; i < 200; i++){
    wallDetectionVal = analogRead(IRInputPin);
    Serial.println(wallDetectionVal);
    delay(10);
  }
  ...
  reset();
  //TODO: MAYBE REMOVE THIS IF THE BOT STARTS WORKING AGAIN
  forward();
  delay(500);
  ...
  if(demoMode == 2){
    ...
    while(!message.endsWith("JAQPETTI RETURNED HOME")){
      messageSize = client.parseMessage();
      if (messageSize > 0) {
        message = client.readString();
      }
